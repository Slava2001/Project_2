...
pub trait BuildFromCfg<T> {
    ...
    fn build(cfg: Map<String, Value>, resources: &mut dyn Manger) -> Result<T, Error>;
}

/// Object builder function [`BuildFromCfg::build`]).
type BuildFunc<T> = fn(Map<String, Value>, &mut dyn Manger) -> Result<T, Error>;
...
impl<T> Builder<T> {
    ...
    pub fn build(&self, mut cfg: Map<String, Value>, res: &mut dyn Manger) -> Result<T, Error> {
        let Some(object_type) = cfg.remove("type") else {
            bail!(Error::msg("Configuration does not contain object type"));
        };
        let object_type = object_type
            .into_string()
            .change_context(Error::msg("Failed to parse as object type"))?;
